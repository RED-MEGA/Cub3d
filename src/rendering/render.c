...
/*   Updated: 2023/09/10 14:38:52 by azarda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int calcul_ofset_x(t_ray ray, t_img img)
{
	int ofset_x;
		if(ray.flag == 1)
			ofset_x = (ray.pos.x / SQUARE_LEN - (int)ray.pos.x / SQUARE_LEN) * img.width;
	else if (ray.flag == 2)
			ofset_x = (ray.pos.y / SQUARE_LEN - (int)ray.pos.y / SQUARE_LEN) * img.width;
	return (ofset_x);
}

// void	loup_rendering_ray()
// {
